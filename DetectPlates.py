...
def detectPlatesInScene(imgOriginalScene):
    listOfPossiblePlates = []                   # variabel hasil

    height, width, numChannels = imgOriginalScene.shape # ambil shape dari frame berupa tinggi lebar dan numchannel
    ...
    imgGrayscaleScene, imgThreshScene = Preprocess.preprocess(imgOriginalScene)         # Preprocess untuk dapatkan grayscale dan thres
    ...
    # end if # show steps #########################################################################
            ...
            # Pencarian semua kemungkinan karakter pada tampilan
            # Deteksi kontur, ambil kontur yang akan menjadi kemungkinan karakter (tanpa ocr)
...
def findPossibleCharsInScene(imgThresh):
    listOfPossibleChars = []                # return value

    intCountOfPossibleChars = 0

    # copy thres, deteksi kontur akan merubah gambar
    imgThreshCopy = imgThresh.copy()

    _, contours, npaHierarchy = cv2.findContours(imgThreshCopy, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)   # deteksi semua kontur
    height, width = imgThresh.shape

    #black = cv2.imread("D:\\Wiranda\\Kuliah\\Skripsi\\Penulisan\\Data Gambar\\black.jpg")
